...
class HestonICPINN:
    ...
    Trial solution: U = phi(S,v,tau) + tau*v * NN(S,v,tau)
      - psi = tau*v vanishes at tau=0 (terminal) and v=0 (degenerate BC)
    ...
    def _pretrain_aux(self, epochs=10000, n=5000, lr=1e-3):
        ...
        for ep in range(1, epochs + 1):
            ...
            tau_T = self._to(torch.zeros(n, 1))                  # tau=0
            S_n, v_n, tau_n = self._normalise(S_T, v_T, tau_T)
            target = torch.clamp(S_T / self.K - 1.0, min=0.0)   # K=1 units
    ...
    def train(self, epochs=20000, log_every=1000,
        ...
        for epoch in range(1, epochs + 1):
            ...
            # S*dU/dS + kappa*theta*dU/dv - r*U - dU/dtau = 0
