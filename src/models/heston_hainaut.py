...
Default evaluation parameters (Table 6):
  kappa=1.15 (mean-reversion speed), r=0.04, theta=0.202 (long-term variance),
  xi=0.20, rho=-0.40
  NOTE: paper uses gamma for long-term variance; we map gamma->theta in code.
"""
...
class SkipNet(nn.Module):
    ...
    def __init__(self, input_dim=9, hidden=256, depth=4):
        ...
        # Intermediate layers: (hidden + input_dim) -> hidden  [skip connection]
...
class HestonHainaut:
    ...
    # Training parameter ranges (Table 1 in paper)
    # Paper notation: kappa=mean-reversion speed, gamma=long-term variance
    # Standard Heston: kappa=mean-reversion speed, theta=long-term variance
    S_RANGE    = (20.0,   180.0)
    V_RANGE    = (0.032,  0.52)
    R_RANGE    = (0.01,   0.07)
    K_RANGE    = (0.5,    2.0)    # kappa: mean-reversion speed (paper Table 1: kappa in [0.5,2])
    TH_RANGE   = (0.062,  0.42)   # theta: long-term variance (paper Table 1: gamma in [0.062,0.42])
    XI_RANGE   = (0.1,    0.9)
    RHO_RANGE  = (-0.8,   0.8)
    T_RANGE    = (0.1,    5.0)    # avoid T=0

    K_STRIKE   = 100.0            # fixed strike
    ...
    def __init__(self, device=None):
        ...
        # Z-score stats computed from training distribution (set in _compute_stats)
        self.mean_ = None
        self.std_  = None
    ...
    def _sample_params(self, n):
        """Sample n random parameter combinations from training ranges."""
    ...
    def _compute_stats(self, n=50000):
        """Compute z-score mean/std from a large sample of training inputs."""
    ...
    def _normalise_input(self, t, S, V, r, kappa, theta, xi, rho, T):
        """Z-score normalise the 9-dim input vector."""
        raw = torch.cat([t, S, V, r, kappa, theta, xi, rho, T], dim=1)
    ...
    def _pde_residual(self, t, S, V, r, kappa, theta, xi, rho, T):
        """
        Heston PDE residual (backward time: t=T is now, t=0 is expiry).
        b_h * dF/dt - r*F + (r - V/2)*S*dF/dS + kappa*(theta-V)*dF/dV
          + 0.5*V*S^2*d2F/dS2 + rho*xi*V*S*d2F/dSdV + 0.5*xi^2*V*d2F/dV2 = 0
        b_h = 1/T_max (time scaling factor, here T_max=5)
        """
        ...
        res = (b_h * F_t
               ...
               + rho * xi * V * S * F_SV
               + 0.5 * xi ** 2 * V * F_VV)
        return res / self.K_STRIKE   # normalise to O(1)
    ...
    def train(self, log_every=500):
        ...
        for phase_idx, (lr, n_epochs) in enumerate(phases):
            ...
            for _ in range(n_epochs):
                ...
                # --- Lower boundary loss (S=0, put = K*exp(-r*(T-t))) ---
                ...
                # time remaining = T - t (in original scale, b_h*(T_n - t_n) = (T-t)/T_max)
