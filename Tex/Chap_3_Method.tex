...
软约束方式将终值条件纳入损失函数，网络只能近似满足。参考Guo等人\cite{guo2024icpinn}的ICPINN方法，本文通过两种机制使终值条件近似硬编码：（1）加法参数化中，当$t=T$时，$\tau=T-t=0$，Black-Scholes公式自动退化为终值条件$V_\text{BS}(S,0;\sigma_\text{eff})=\max(S-K,0)$；（2）网络最后一层的小初始化（$\mathcal{N}(0,0.001^2)$）确保修正量$K\cdot\text{net}$在训练初期接近0，不破坏终值条件的满足。两者结合使终值条件在训练开始时即被近似满足，加速收敛。
...
本文引入LLM作为智能路由层，使用户可以通过自然语言描述期权需求，由LLM自动完成模型选择和参数提取。
